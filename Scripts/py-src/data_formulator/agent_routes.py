...
# add this helper near the top (after imports) GPT-5
def resolve_session_id() -> str | None:
    """
    Resolve session id from cookie, header, query or Flask session.
    ...
    if not sid:
        current_app.logger.info("No session_id in request; cookies=%s headers_sample=%s",
                                dict(request.cookies), {k: request.headers.get(k) for k in ("Cookie","Authorization","X-Session-Id")})
    return sid
...
def process_data_on_load_request():
    ...
    if request.is_json:
        ...
        # resolve session id safely GPT-5
        sid = resolve_session_id()
        if not sid:
            return jsonify({"status":"error","error":"session_id missing"}), 401


        conn = db_manager.get_connection(sid)#session['session_id']
        try:
...
def derive_data():
    ...
    if request.is_json:
        ...
        if language == "sql":
            ...
            if not sid:
                #return jsonify({"status":"error","error":"session_id missing"}), 401
                return flask.jsonify({ "token": token, "status": "error", "results": [], "error": "session_id missing" }), 401  

        conn = db_manager.get_connection(sid) if language == "sql" else None
