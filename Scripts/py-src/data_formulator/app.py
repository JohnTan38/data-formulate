...
from data_formulator.db_manager import get_postgres_connection
...
# enable CORS: set ALLOWED_ORIGINS to comma-separated origins, or "*" to allow all
_allowed = os.getenv("ALLOWED_ORIGINS", "*").strip()
if _allowed == "*" or _allowed == "":
    # wildcard: no credentials support
    CORS(app)
else:
    origins = [o.strip() for o in _allowed.split(",") if o.strip()]
    # allow cookies/credentials for specified origins
...
if _allowed.strip() == "*":
    # wildcard does not support credentials
    CORS(app)
else:
    origins = [o.strip() for o in _allowed.split(",") if o.strip()]
    # allow cookies/credentials for specified origins
...
def ensure_session_id():
    ...
    if not sid:
        # generate one and attach to response; handler endpoints should use request context to read cookie
        new_sid = str(uuid.uuid4())
        # attach new cookie for the response by storing for after_request
        request.environ["new_session_id"] = new_sid

@app.after_request
def set_session_cookie(response):
    new_sid = request.environ.get("new_session_id")
    if new_sid:
        # For cross-site usage (frontend on Vercel) set SameSite=None and Secure
        response.set_cookie("session_id", new_sid, httponly=True, samesite="None", secure=True)
    return response
# ...existing code...


@app.route("/_internal/session-create", methods=["POST"])
def session_create():
    sid = str(uuid.uuid4())
    resp = jsonify({"session_id": sid})
    # set cookie suitable for cross-site scenarios
    resp.set_cookie("session_id", sid, httponly=True, samesite="None", secure=True)
    return resp
...
def db_test():
    try:
        ...
        # return error so you can see it in HTTP response and Cloud Run logs
