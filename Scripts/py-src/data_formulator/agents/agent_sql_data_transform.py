...
def get_sql_table_statistics_str(conn, table_name: str, 
    ...
    # Ensure table name is sanitized and build a properly quoted identifier
    table_name = sanitize_table_name(table_name)

    def quote_ident(name: str) -> str:
        """
        Quote SQL identifiers safely for DuckDB.
        - Handles qualified names like schema.table by quoting each part.
    ...
    try:
        ...
        logger.exception("Failed to DESCRIBE table %s. Query: DESCRIBE %s", table_name, quoted_table)
        raise

    try:
        sample_data = conn.execute(f"SELECT * FROM {quoted_table} LIMIT {row_sample_size}").fetchall()
    except Exception as e:
        logger.exception("Failed to SELECT sample rows from %s. Query: SELECT * FROM %s LIMIT %s", table_name, quoted_table, row_sample_size)
        raise
    ...
    for i, row in enumerate(sample_data):
        formatted_sample_data += f"{i}| " + " | ".join(
            (str(val)[:max_val_chars] + "..." if len(str(val)) > max_val_chars else str(val))
            for val in row
        ) + " |\n"
    ...
    for col in columns:
        ...
        quoted_col_name = quote_ident(col_name)

        # Basic stats query (use quoted identifiers)
        if col_type in ['INTEGER', 'DOUBLE', 'DECIMAL', 'BIGINT', 'FLOAT', 'REAL', 'NUMERIC']:
            stats_query = f"""
            SELECT 
                COUNT(*) as count,
        ...
        try:
            ...
            logger.exception("Error executing stats query for column %s in table %s.\nQuery:\n%s", col_name, table_name, stats_query)
            # Fallback: provide minimal stats info instead of raising
            col_metadata_list.append({
                "column": col_name,
                "type": col_type,
                "statistics": {"error": f"Failed to compute stats: {type(e).__name__}"}
            })
            continue
        ...
        if col_type in ['INTEGER', 'DOUBLE', 'DECIMAL', 'BIGINT', 'FLOAT', 'REAL', 'NUMERIC']:
            ...
            # Combined query for distinct sample values
            # Quote identifier and limit results
            ...
            try:
                ...
                stats_dict['sample_values'] = [
                    (str(val[0])[:max_val_chars] + "..." if len(str(val[0])) > max_val_chars else str(val[0]))
                    for val in sample_values
                ]
            except Exception as e:
                logger.exception("Error getting sample values for column %s in table %s.\nQuery:\n%s", col_name, table_name, query_for_sample_values)
                stats_dict['sample_values'] = []
